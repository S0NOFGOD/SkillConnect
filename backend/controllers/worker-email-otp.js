...
console.error("Worker email OTP verification error:",error);
return res.status(500).json({success:false,message:"An error occurred while verifying your email."});
}
};
...
return res.status(500).json({success:false,message:"An error occurred while sending the verification code."});
}
};

module.exports={verifyEmailOTP,resendEmailOTP};